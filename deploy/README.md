...
`/metrics` is served on a **listener of its own**, not on the API port — set `metrics.api_port` and
`metrics.worker_port` (the example config uses `9464` and `9465`, which is what `prometheus.yml`
scrapes). Two keys, because one `enclave.yaml` is read by both processes and a shared port meant
whichever started second died with `Address already in use`. Both are off by default and bind to
`metrics.bind`, which is loopback. The exposition carries `tenant_id` labels, so it must never be
reachable from where the API is; [`docs/11-OPERATIONS.md §10.1`](../docs/11-OPERATIONS.md) has the
reasoning and the deployment rule. If a scrape is refused, the usual cause is that the port was left
unset.

One honest gap remains, written here rather than discovered during an incident:
...
The second gap is closed: `enclave-worker` binds `metrics.worker_port` and serves the coverage
probe's gauges there, so its scrape job is a live target rather than a placeholder.
