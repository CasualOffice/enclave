...
| R3 | ~~**A scanned PDF is not searchable by its content.**~~ **Closed.** The join this row named as missing is built: `ENC-545`'s `PdfTextExtractor` reports the pages that yielded nothing, `crates/worker/src/main.rs` routes `application/pdf` to it when PDFium is mounted, and `crates/worker/src/indexing.rs` runs `MountedOcr::retry` on `Outcome::NoText` before writing chunks and vectors (`ENC-546`) | Kept rather than deleted, because *what closed it* is the useful part: two named, bounded pieces of wiring, exactly as this row predicted. `§4` carries the evidence and the two qualifications that remain |
...
| Drop ratio and denylist size exported, alerts wired | **HALF MET, and this row said `Met` until `ENC-991` read it against the tree.** The drop ratio is real: `PostFilter::confirm` publishes every pass through `crates/search/src/postfilter.rs::publish`, and `deploy/monitoring/alerts/search.yml` holds the single recording rule and both alerts. **The denylist size is not exported at all** — `enclave_observability::metrics::search::record_denylist_size` has no caller anywhere outside `metrics.rs`'s own unit tests, so `enclave_search_denylist_entries` and `_limit` never exist and `SearchDenylistBacklogGrowing` / `SearchDenylistOverflowedAndTenantIsDegraded` are structurally incapable of firing. The rule file's own `SearchDenylistSizeUnreported` describes this deployment exactly, and would be firing. **No tracker row covers it.** The listener this row was written about is real and is not the question |
| A scanned, text-free PDF is searchable by its content | **MET 2026-08-22; this row said `NOT MET` until `ENC-991` re-read it, and `plans/M3-DISCOVERY.md §1` had been carrying the tick and the evidence in the meantime.** Every join it named as missing was built: `ENC-545` (`PdfTextExtractor`, returning `NoText` with the pages that yielded nothing), `ENC-546` (`MountedOcr::retry` wired into `crates/worker/src/indexing.rs`), `ENC-577` (`crates/worker/src/main.rs` registers `application/pdf`, only when PDFium is mounted) and `ENC-578` (`text_an_indexing_pass_committed_is_text_lexical_search_finds`, `crates/worker/tests/indexing.rs` — a pass writes rows and a search reads them back in one process). Two qualifications, both already recorded there: **no single test spans all four boundaries** — they are joined pairwise with real components on both sides of each — and OCR is a deployment option, since a worker without the two mounted volumes routes PDFs nowhere and records `SKIPPED` rather than failing, which is deliberate. `TRACKER.md`'s `ENC-161` row still reads `BLOCKED` on a model-file decision `ENC-535` answered |

**M3's remaining gap moved, and it is smaller and quieter than the one this section was written
about.** The scanned-PDF criterion is met; what is not is the denylist-size gauge, which nobody
publishes — an operator signal that reads as *nothing to report* rather than as *nothing reporting*.
The definition of done also requires every M3 P1 to be `DONE`; `ENC-527` is `REVIEW` pending CI, and
the remaining P1s are tracked.

The honest summary: **the authorization properties of the search path are met and were each verified
by breaking them on purpose. The retrieval-completeness properties are now met too** — OCR reaches a
scanned PDF, and what is left is observability rather than retrieval: the index-health signal can see
absence but not wrongness, and two of the denylist alerts have no series to read.
