...
| A scanned, text-free PDF is searchable by its content | **NOT MET.** No OCR. `ENC-161` is blocked on model files and language coverage (Q12) |

**M3 is therefore not complete**, and the gap is a single named criterion with a named blocker. The
