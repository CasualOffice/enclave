...
| ~~Q21~~ | **Answered 2026-08-25: people with access.** ACL data, available now. Presence would need a real-time subsystem M5 does not have, and an avatar stack that silently means *viewing now* on a product with no presence would show a single frozen face — worse than showing nothing. Named limitation so it is not rediscovered: an access stack on a heavily-shared folder is a long list, so it truncates with a count and the full list lives in the peek panel's Access tab | — | Closed |
