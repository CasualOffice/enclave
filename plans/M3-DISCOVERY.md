...
|---|---|---|---|
| ~~Q12~~ | **Answered 2026-08-21.** Engine: `ocrs` + `rten` — pure Rust, MIT OR Apache-2.0 across its tree, and crucially **no `links` key**, so it needs no C toolchain and cannot conflict; both `tesseract` and `leptess` declare `links = "tesseract"` and so cannot coexist in one graph. Languages: **English only** to start. The exit criterion asks that a scanned PDF be searchable, not that every language be covered, and adding languages later is additive rather than a reindex | — | Closed |
...
| ~~Q14~~ | **Answered 2026-08-21.** `bge-m3` at **1024 dimensions**, **mounted at runtime**. Multilingual, because `docs/14` has tenants in many languages and an English-only model fails *silently* on the rest — it does not error, it stops matching, surfacing months later as "search does not work for the Munich office". Natively emits sparse vectors, which fills the `SPARSE_INVERTED_INDEX` field the collection already has and nothing was producing. Costs accepted knowingly: 1024 dims is 4× the index size of a 384-wide model, and it is the slowest per chunk of the candidates. Mounted rather than baked because `docs/08 §18`'s air-gapped installs pay a multi-gigabyte layer on every image pull. Recorded in `enclave_embeddings::model::ACTIVE`, where the id and the width are one value so they cannot drift | — | Closed |
