...
## 12A. Storage tiers (`ENC-946`)

`file_versions` carries `storage_tier` and `restore_requested_at`, added by
`migrations/0032_storage_tier.sql`. Archival appeared nowhere in this document before that
migration — `tier`, `storage class`, `Glacier` and `archive` were unwritten — so the decisions are
...
CONSTRAINT file_versions_restoring_has_a_start
    CHECK (storage_tier <> 'RESTORING' OR restore_requested_at IS NOT NULL)
```

**A tier is not a `status`, and the separation is the design.** `file_versions.status` answers *is
this content processed and safe to serve* — it is the column `CLAUDE.md` rule 9 is about. A tier
answers *how long will fetching it take*. They are orthogonal: a version that is clean, scanned,
approved and cold is `AVAILABLE` **and** `ARCHIVED`.

Folding them costs three things. An archived version would be indistinguishable from a quarantined
one at every `status = 'AVAILABLE'` predicate in the tree, so archiving content would silently make
it look infected. Rule 9's column would begin carrying a latency fact, leaving no way to tell which
of its values are safety and which are speed. And a restore would have to *write* `status`, which
is the antivirus pipeline's column — a second writer to the one place the product cannot afford
two.

**The two transient values are load-bearing.** No provider with a cold tier transitions instantly.
`ARCHIVING` exists so a read is refused from the moment a transition is *requested*, closing the
window in which a signed URL would be minted for an object already on its way to Glacier;
`RESTORING` exists so a rehydration is a state to poll rather than a request that blocks.

**The design assumes slow rehydration.** S3 Glacier Deep Archive is measured in hours. A design
built for minutes breaks on the slow provider; one built for hours degrades to minutes without
changing shape. `restore_requested_at` is not cleared when a restore completes — how long the last
one took is the only evidence a deployment has that its archive tier is behaving, and clearing it
destroys that evidence at the moment it becomes useful.

**Existing rows backfill to `HOT`**, which is what they are: nothing has moved any bytes. The
failure direction decides it — a wrong `HOT` makes the product try to read bytes that are there; a
wrong `ARCHIVED` takes every file in the deployment offline.

`migrations/0033` adds `tier_verified_at` (`ENC-951`): when the object store last confirmed the
tier, `NULL` for never asked.

```sql
tier_verified_at TIMESTAMPTZ
```

It exists because the tier can change **without this product being told**. Most content reaches a
cold tier through a bucket lifecycle rule an operator wrote and the provider executed; those rows
still say `HOT`, and a download mints a signed URL the object store refuses. Detecting that costs
one `HeadObject` per version of every file, so the scan is bounded per tick and has to be *fair* —
which needs a column to order by. `NULL` sorts first, so the existing corpus goes to the front of
the queue rather than being treated as checked at migration time.

**It is not evidence the tier is correct now**, only that it was then, which is all an observation
can establish. The number that matters is the **oldest** value in the table — the deployment's
worst-case staleness — and `crates/worker/src/tiering.rs` reports it every pass, because a scan
whose coverage nobody can state is a scan nobody can trust.

## 12B. Favorites (`ENC-959`)

`migrations/0034` creates `favorites`. *Favorites* had carried a `Later` chip in the navigation
since the shell was written and existed nowhere in this schema — `favorite`, `starred` and
`bookmark` appeared in no document and no migration — so it is recorded here as the new product
decision it is.
...
**It is the user's own data, and that shapes every column.** A favorite grants nothing, reveals
nothing and decides nothing: it is a private note somebody made about a file they could already
see. Two people starring one file are two rows that cannot see each other, and no other table
references this one. That is why the natural key is all three columns — starring twice is the same
fact, not two of them, which is what makes the write idempotent without a read first.

**`DELETE` is granted, and that is the exception worth stating.** `retention_assignments`,
`dlp_rules` and `print_tokens` all withhold it, because a statement that removes the evidence a
control existed is the statement those tables are for. None of that applies to a star: there is no
compliance value in the record that somebody once favourited a document, and withholding the verb
