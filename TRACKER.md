...
| ENC-503 | CI pulls ClamAV anonymously, so a registry change reads as a security-test failure | P2 | TODO | Found while closing `ENC-140`. Anonymous Docker Hub pulls work today — 100 per six hours per source IP, stated inside the token — so the dev stack needs no login. The exposure is CI: `.github/workflows/ci.yml` pulls `clamav/clamav:1.4` anonymously and runs the EICAR tests, so if Hub closes anonymous pulls or a busy runner IP exhausts the budget, **leakage row G1 fails and looks like a security defect rather than a registry one**. No mirror exists — quay, ghcr and ECR all checked. Wants either a cached image or a failure message that names the cause |
