...
| ENC-517 | `CREATE INDEX CONCURRENTLY` needs one statement per migration | P3 | DONE | Migration 0004 predicted the `-- no-transaction` half of this in advance; the other half cost a debugging round. PostgreSQL wraps *multiple* statements sent in one round trip in an implicit transaction block, so two `CONCURRENTLY` builds in one file fail with `25001` — the same error as running inside an explicit transaction, raised from a file that had carefully opted out of one. Hence 0012 and 0013 as separate migrations, each saying why |
