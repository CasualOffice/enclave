...
| ENC-172 | Editing an unmerged migration leaves a persistent dev database unusable | P3 | TODO | Surfaced three times while verifying ENC-155. The harness applies migrations to the `DATABASE_URL` database as well as to its throwaway one, so an edit to a migration that has already been applied there fails every later run with `Migrate(VersionMismatch(9))` — correct behaviour from the forward-only checksum gate, and an opaque message for the one case where editing is legitimate: a migration that is not merged yet. The remedy today is `DELETE FROM _sqlx_migrations WHERE version = n` plus dropping the tables by hand. Wants either a harness that never touches the shared database, or an error that says which migration and what to do |
