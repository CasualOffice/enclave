...
| ENC-172 | Editing an unmerged migration leaves a persistent dev database unusable | P3 | DONE | `VersionMismatch(n)` — a variant name and an integer — now becomes `MigrationModified`, naming the migration and giving the remedy verbatim. Nothing is weakened: it still fails, and the checksum comparison behind it *is* the forward-only gate. A test asserts the message sends someone with a *merged* migration forward to a new one rather than backward, so a legible error does not become a documented way round the gate |
...
| ENC-176 | `ttf-parser` is unmaintained, reached through `ab_glyph` | P2 | TODO | Accepted in `deny.toml` with the reasoning, not waved through. An unmaintained *parser* is normally a serious finding here — D17 exists because parsers are the widest attack surface in this product — and this one is different in the way that matters: it never sees attacker-controlled input. The only font it parses is the one vendored in this repository and compiled in with `include_bytes!`; what a viewer controls is the text, which our own code bounds. The acceptance ends when `ab_glyph` moves to a maintained parser **or when the watermark needs a font we do not ship**, because that is the moment the argument above stops holding. Revisit 2026-11-20 |
