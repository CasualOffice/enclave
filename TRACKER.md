...
| ENC-146a | A real renderer, in a sandboxed worker process | P1 | TODO | Split out of ENC-146 once the pipeline was written: the codecs are where the attack surface actually is, they run out-of-process per D17, and none of the pipeline's guarantees depend on which one is plugged in. `NoRenderer` refuses everything until then, in the shape `crates/core`'s deny-by-default policy stubs use. Blocks leakage rows G3 and G4 |
| ENC-155 | `sqlx::migrate!` embeds migrations at compile time, so a schema gate can pass against a stale build | P2 | TODO | Found while proving the RLS gate covers `renditions`: editing a `.sql` and re-running the gate reported 30 tables and **passed**, because `enclave-db` had not been rebuilt and its embedded copy still held the old file. CI builds from scratch and is unaffected; locally it is a gate that reports green on a schema nobody is running. Wants a `rerun-if-changed` on `migrations/`, or a gate that reads the directory rather than the binary |
