...
| ENC-155 | `sqlx::migrate!` embeds migrations at compile time, so a schema gate can pass against a stale build | P1 | TODO | Found while proving the RLS gate covers `renditions`: editing a `.sql` and re-running the gate reported 30 tables and **passed**, because `enclave-db` had not been rebuilt and its embedded copy still held the old file. CI builds from scratch and is unaffected; locally it is a gate that reports green on a schema nobody is running. Wants a `rerun-if-changed` on `migrations/`, or a gate that reads the directory rather than the binary. **Recurred during ENC-151**, costing two rounds of confused debugging against "relation does not exist" on a table the migration plainly creates — so it is a trap that catches the person who logged it, not just the next person. Promoted from P2 to P1 |
...
| ENC-169 | Composite the watermark server-side so previews of marked content work | P1 | TODO | `ENC-148` refuses a preview carrying `Obligation::Watermark`, which is the safe direction and leaves the sensitive content — exactly the content policy watermarks — without previews. `crates/preview` composes the SVG layer already (`ENC-147`); what is missing is rasterising it over the PNG. The tempting alternative is not a control: sending the base and the overlay separately and letting the client combine them means a client that does not draw the overlay gets an unmarked page |
| ENC-170 | The binary registers two routes whose dependencies it never provides | P1 | TODO | `router()` wires `POST /files/{id}/download` and now `GET /files/{id}/preview`, both of which take an axum `Extension` — a `BlobStore` and a `PreviewPipeline`. `main.rs` constructs neither, so both return `500` in the binary while passing every integration test, because the tests build their own router with the extensions attached. Pre-existing for download since PR #22 and invisible because nothing runs the binary against a real request. The fix is not to add two `.layer()` calls: it is for `router()` to *take* what its routes need, so a route cannot be registered without it and the gap becomes a compile error |
...
| ENC-167 | `authorize_many`'s cost is ~80% fixed, which inverts an M3 design assumption | P1 | TODO | Measured in ENC-145: one candidate 1.4 ms, two hundred 7.0 ms. The post-filter's price is transaction setup plus three round trips, not candidate count. So raising `OVERFETCH` is nearly free, while a *second* `authorize_many` call — which `docs/07 §6.2` describes for resolving excerpt disclosure separately from result disclosure — costs more than tripling the batch. M3 should decide whether both disclosure levels can be resolved in one call before the search design sets. Separately, `authorize_many` batches resources but not actions, so a capabilities page costs one call per action (ENC-152); an `action = ANY($1)` form would collapse those to one |
