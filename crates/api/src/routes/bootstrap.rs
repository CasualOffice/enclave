...
//!
//! # The unauthenticated half carries nothing tenant-derived, and cannot
//!
//! Not as a policy choice but as a consequence: resolving a host to a tenant needs
//! `enclave_db::resolve_routed_tenant`, which does not exist in this tree, and the fallback route —
//! reading `tenant_domains` — is closed by construction, because that table *is* RLS-policied and
//! is therefore unreadable before a tenant context exists (`migrations/0002_rls_policies.sql` says
//! so, and asks for exactly this decision). So [`PublicBootstrap`] is byte-identical for every
//! `Host`, which `crates/api/tests/bootstrap.rs` asserts against alpha's host, beta's host and an
//! unknown one.
//!
//! When `RoutedTenant` lands, this module consumes it. It does not grow a second host-to-tenant
//! mechanism of its own — two answers to one question is how the two drift, and the drift is
//! silent. `ENC-730`.
...
fn best_accepted(header_value: &str) -> Option<&'static str> {
    ...
    for (position, part) in header_value.split(',').enumerate() {
        ...
        for parameter in fields {
            ...
            if let Some(value) = parameter.strip_prefix("q=").or_else(|| parameter.strip_prefix("Q="))
            {
...
fn best_match(tag: &str) -> Option<&'static str> {
    ...
    if let Some(hit) = SUPPORTED_LOCALES.iter().find(|candidate| candidate.eq_ignore_ascii_case(tag))
    {
        return Some(hit);
    }
    ...
    SUPPORTED_LOCALES
        .iter()
        .copied()
        .find(|candidate| candidate.split('-').next().is_some_and(|p| p.eq_ignore_ascii_case(primary)))
}
